...
Entity World::GetEntity(EntityID entity)
{
	...
	{
		// Entity list doesn't contain entity, check entity manager
		if (m_EntityManager->entities()[entity])
			// Entity manager has entity, add it to the entity list (lists were out of sync?)
			m_Entities.emplace(entity, Entity(entity, this));
	}
	return m_Entities.at(entity);
}

bool World::EntityExists(EntityID entity) { return m_Entities.find(entity) != m_Entities.end(); }
