...
// --- MAPS --- //
unordered_map<ResourceID, Sound> ResourceManager::m_Sounds;
unordered_map<ResourceID, Texture> ResourceManager::m_Textures;

unordered_map<string, ResourceID> ResourceManager::m_ResourcePaths;
...
ResourceID ResourceManager::LoadSound(std::string filename)
{
	...
	if (m_ResourcePaths.find(filename) != m_ResourcePaths.end())
		return m_ResourcePaths[filename]; // Already exists
...
Sound ResourceManager::GetSound(ResourceID id) { return (id == InvalidResourceID || (m_Sounds.find(id) == m_Sounds.end())) ? Sound{} : m_Sounds[id]; }
...
{
	return (id == InvalidResourceID || m_Textures.find(id) == m_Textures.end()) ? Texture{ 0 } : m_Textures[id];
}
...
ResourceID ResourceManager::GetNextResourceID()
{
	PROFILE_FN()
	ResourceID ID = 0;
	while (m_Sounds.find(ID) != m_Sounds.end() || m_Textures.find(ID) != m_Textures.end())
		ID++;
	return ID;
}
